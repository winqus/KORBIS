...
Deno.test("WeaviateV2ItemsRepository", async (t) => {
  ...
  await t.step("should handle class not found error during createWithImage", async () => {
    ...
    stub(mockClient.data.creator().withClassName().withProperties(), "do", () => {
      if (!initCalled) {
        initCalled = true;
        return Promise.reject(new Error('Error: usage error (422): {"error":[{"message":"invalid object: class \\"Item\\" not found in schema"}]}'));
      }
  ...
  await t.step("should search items by text", async () => {
    ...
    const items = await repo.search({ queryText: "test" });
  ...
  await t.step("should search items by image", async () => {
    ...
    const items = await repo.search({ queryImageBase64: "base64image" });
  ...
  await t.step("should search items by text and image", async () => {
    ...
    const items = await repo.search({ queryText: "test", queryImageBase64: "base64image" });
  ...
  await t.step("should add a file to an item", async () => {
    ...
    stub(repo, "findById", () => Promise.resolve({
      id: "test-id",
      name: "Test Item",
      description: "Test Description",
      ownerId: "user123",
  ...
  await t.step("should throw DocumentNotFoundError when item not found during addFile", async () => {
    const mockClient = createMockWeaviateClient();
    const repo = new WeaviateV2ItemsRepository(mockClient as any);
  ...
  await t.step("should throw DocumentNotFoundError when file not found during deleteFile", async () => {
    const mockClient = createMockWeaviateClient();
    const repo = new WeaviateV2ItemsRepository(mockClient as any);
  ...
  await t.step("should throw DocumentNotFoundError when file undefined during deleteFile", async () => {
    const mockClient = createMockWeaviateClient();
    const repo = new WeaviateV2ItemsRepository(mockClient as any);
  ...
  await t.step("should throw DocumentNotFoundError when item not found during getFiles", async () => {
    const mockClient = createMockWeaviateClient();
    const repo = new WeaviateV2ItemsRepository(mockClient as any);
